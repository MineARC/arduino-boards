...
void RTCZero::begin(Clock_Speed div, bool resetTime) {
  uint16_t tmp_reg = 0;
...
void RTCZero::configureClock(Clock_Speed div) {
  ...
  GCLK->GENCTRL.reg = (GCLK_GENCTRL_GENEN | GCLK_GENCTRL_SRC_OSCULP32K | GCLK_GENCTRL_RUNSTDBY | GCLK_GENCTRL_ID(4) |
                       GCLK_GENCTRL_DIVSEL);
#else
  GCLK->GENCTRL.reg = (GCLK_GENCTRL_GENEN | GCLK_GENCTRL_SRC_XOSC32K | GCLK_GENCTRL_RUNSTDBY | GCLK_GENCTRL_ID(4) |
                       GCLK_GENCTRL_DIVSEL);
#endif
  while (GCLK->STATUS.reg & GCLK_STATUS_SYNCBUSY)
    ;
...
#ifndef CRYSTALLESS
  SYSCTRL->XOSC32K.reg = SYSCTRL_XOSC32K_ONDEMAND | SYSCTRL_XOSC32K_RUNSTDBY | SYSCTRL_XOSC32K_EN32K |
                         SYSCTRL_XOSC32K_XTALEN | SYSCTRL_XOSC32K_STARTUP(6) | SYSCTRL_XOSC32K_ENABLE;
#endif
}
